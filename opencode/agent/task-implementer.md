---
description: |
  Systematically implement tasks from a task plan or tracking file.
  Trigger this agent when: 
    
  1) A user explicitly asks to 'implement the next task' or 'work on the task list'
  2) A user references a task plan file and requests implementation
  3) After completing a code change when a task plan exists in the project
  4) When the user provides a path to a markdown task file and wants automated 
     task execution.
mode: subagent
---
